...
export default function Header({ stats }) {
  ...
  const runs = Object.values(stats?.last_runs || {});
  const anyRunning = runs.some((r) => r.status === "running") || lastRun?.status === "running";
  const allFailed = runs.length > 0 && runs.every((r) => r.status === "error");
  return (
      ...
      <div className="container header__inner">
        ...
        <div className="header__status">
          {lastRun ? (
            <>
              <span
                className={`led ${anyRunning ? "led--busy" : allFailed ? "led--alert" : "led--ok"}`}
                aria-hidden="true"
              />
