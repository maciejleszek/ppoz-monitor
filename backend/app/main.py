...
@app.get("/api/debug/ted")
def debug_ted():
    """Diagnostyka połączenia z TED Search API (przetargi unijne)."""
    from .scraper import ted
    return ted.probe()


@app.get("/api/debug/bk")
def debug_bk():
    """Diagnostyka połączenia z API Bazy Konkurencyjności."""
    from .scraper import bk
    return bk.probe()
...
def stats(db: Session = Depends(get_db)):
    ...
    # ostatni przebieg KAŻDEGO źródła (nagłówek strony agreguje ich statusy)
    last_runs: dict[str, LastRun] = {}
    for row in db.query(ScrapeRun).order_by(desc(ScrapeRun.started_at)).limit(30):
        if row.source not in last_runs:
            last_runs[row.source] = LastRun.model_validate(row)
