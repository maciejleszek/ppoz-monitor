...
Codziennie pobiera ogłoszenia z trzech źródeł: **Biuletynu Zamówień Publicznych** (oficjalne, publiczne API platformy e-Zamówienia prowadzonej przez UZP), **TED** (przetargi unijne powyżej progów — oficjalne Search API v3) oraz **Bazy Konkurencyjności** (zapytania ofertowe beneficjentów funduszy UE, często poniżej progów Pzp — czyli zamówienia, których w BZP nie ma). Wszystko filtruje słownikiem branżowym PPOŻ (SSP, oddymianie, hydranty, gaśnice, DSO, kody CPV itd.) i prezentuje w panelu z wyszukiwarką, filtrami województw i terminami składania ofert.
...
| `ENABLE_TED` | `true` | Źródło TED (przetargi unijne, Search API v3). |
| `ENABLE_BK` | `true` | Źródło Baza Konkurencyjności (zapytania ofertowe z projektów UE). |
...
- `GET /api/debug/bzp`, `/api/debug/ted`, `/api/debug/bk` — diagnostyka połączeń ze źródłami (kody HTTP prób, działający wariant, rzeczywiste pola rekordów),
...
- Moduł TED korzysta z oficjalnego, publicznego Search API v3 (`api.ted.europa.eu`) — odczyt opublikowanych ogłoszeń nie wymaga klucza API.
- Moduł Bazy Konkurencyjności czyta zaplecze REST serwisu (`/api/`), którym posługuje się sama aplikacja BK2021; endpoint nie jest formalnie udokumentowany, więc moduł sonduje kilka wariantów, a jego stan sprawdzisz zawsze pod `GET /api/debug/bk`. Dane są jawne z mocy zasady konkurencyjności (Wytyczne kwalifikowalności 2021–2027).
